...
def login():
  ...
  curHash = HashPasswords.calcPepperHash('pswd'.encode(), sal, pep)
  print('\n*Email is NOT case-sensitive*',end='')
  while (numGuesses > 0):
    email = input("\nEnter Email Address: ")
    if email.lower() == 'quit':
      inp = input("You entered " + email + " ... are you trying to quit the program? (y/n) ")
    ...
    curHash = HashPasswords.calcPepperHash(usrin, sal, pep)
    break
...
def addContact(hash):
  ...
  if tempName == 'q':
    print("Name ignored ... returning back to SecureDrop")
    return
  ...
  if tempEmail == 'q':
    print("Name and email ignored ... returning back to SecureDrop")
    return
...
def formatNumUsers() -> str:
  ...
  numU = getNumUsers()
  if numU == 1 :
    return "There is 1 user registered.\n"

  return "There are " + str(numU) + " users registered.\n"
...
def isCustomReceive(userin, hash, usr_email) -> bool:
  try:
    ...
    if len(args) == 2 and type(int(args[1])) == int:
      receiver.receive_file(hash, usr_email, TimeOut=int(args[1]))
      return True
  except:
    return False
...
def secureDrop():
  ...
  while True:
    ...
    if userinput == "help":
      ...
      sender.send_file(hash, usr_email)

    elif(userinput == "receive"):
      receiver.receive_file(hash, usr_email)
